...
def transform_images(dir_path):
    ...
    """
    image_paths = [join(dir_path, file_path) for file_path in listdir(dir_path)
                   if isfile(join(dir_path, file_path)) and '.png' in file_path]
    image_path_tuples = [(image_path, int(re.search('.*\\\\(.*)\.png', image_path).group(1)))
                         for image_path in image_paths]
    image_path_tuples.sort(key=lambda x: x[1])
    image_paths = [image_path_tuple[0] for image_path_tuple in image_path_tuples]
